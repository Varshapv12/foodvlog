...
def cart_details(request,tot=0,count=0,cart_items=None):
    try:
        ...
        pass
    return render(request,'cart.html',{'t':tot,'cn':count,'ci':cart_items,})
...
def add_cart(request,product_id):
    ...
    try:
        ...
        ct.save()
    try:
        c_items=items.objects.get(prodt=prod,quan=1,cart=ct)
...
def min_cart(request,product_id):
    ...
    c_items=cartlist.objects.get(prodt=prod,cart=ct)
    if c_items.quan>1:
        c_items.quan-=1
        c_items.save()
    else:
